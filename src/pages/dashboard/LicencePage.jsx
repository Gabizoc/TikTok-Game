...
const LicensesPage = () => {
  ...
  useEffect(() => {
    const fetchLicense = async () => {
      try {
        const response = await axios.get("/api/user/license");
        setUserLicense(response.data.license);
      } catch (error) {
        console.error("Erreur lors de la récupération de la licence :", error);
        showNotification(t("Failed to fetch your license"), "error");
      }
    };
